 ...
 * Hand-maintained (not generated) — wraps Gizmo's v3 Server-Sent Events endpoint.
 *
 * Usage:
...
export class EventStream {
  ...
  filters: number[];
  eventSource: EventSourcePlus | null;
  controller: any;
  onmessage: ((data: any) => void) | null;
  onerror: ((response: any) => void) | null;
  onabort: ((reason?: any) => void) | null;

  /**
   * Create a new EventStream instance
   * @param client - The Gizmo API client
   */
  constructor(client: GizmoClient, options: EventStreamOptions = {}) {
    this.client = client;
   ...
   * @returns The EventSource URL with current filters
   */
  _getEventStreamUrl(): string {
    let url = `${this.client.baseURL}/api/v3/events/stream`;
   ...
   */
  async run(onmessage: (data: any) => void): Promise<this> {
   ...
   * @private
   */
  _createEventSource(onmessage: (data: any) => void): this {
   ...
   * @returns {EventStream} Returns this for method chaining
   */
  onError(onerror: (response: any) => void): this {
  ...
  onAbort(onabort: (reason?: any) => void): this {
   ...
   * @returns True if EventSource is active
   */
  isRunning(): boolean {
